...
da_unadj <- read_tsv(paste0(data_path_tophits, "dense_area_unadj_tophits.tsv")) # 21
nda_unadj <- read_tsv(paste0(data_path_tophits, "nondense_area_unadj_tophits.tsv")) # 8
pd_unadj <- read_tsv(paste0(data_path_tophits, "percent_density_unadj_tophits.tsv")) # 11
...
length(intersect(da_unadj$SNP , da_new$SNP)) # 8
length(intersect(nda_unadj$SNP , nda_new$SNP)) #1
length(intersect(pd_unadj$SNP , pd_new$SNP)) # 1
...
## extract from adj and unadjust

# da
da_new_all <- vroom("/Users/ny19205/OneDrive - University of Bristol/Documents - OneDrive/Mini-project4/01_Data/GWAS_tidy/dense_area_GWAS_tidy_outcome.txt.gz") %>% select(SNP, beta_adj = beta.outcome, pval_adj = pval.outcome)
da_unadj_all <- vroom("/Users/ny19205/OneDrive - University of Bristol/Documents - OneDrive/Mini-project4/01_Data/GWAS_tidy/dense_area_unadj_GWAS_tidy_outcome.txt.gz")%>% select(SNP, beta_unadj = beta.outcome, pval_unadj = pval.outcome)

da_merged<- full_join(da_new_all, da_unadj_all) %>% 
            filter(grepl("rs", SNP)) %>% 
            mutate(adj = ifelse(SNP %in% da_new$SNP, 1, 0)) %>% 
            mutate(unadj = ifelse(SNP %in% da_unadj$SNP, 1, 0)) %>% 
            filter(adj==1 | unadj==1)
write_tsv(da_merged, paste0(results_path, current_trait_category, "/compare_DA_adj_unadj.tsv"))
da_merged<- read_tsv(paste0(results_path, current_trait_category, "/compare_DA_adj_unadj.tsv"))

# nda
nda_new_all <- vroom("/Users/ny19205/OneDrive - University of Bristol/Documents - OneDrive/Mini-project4/01_Data/GWAS_tidy/nondense_area_GWAS_tidy_outcome.txt.gz") %>% select(SNP, beta_adj = beta.outcome, pval_adj = pval.outcome)
nda_unadj_all <- vroom("/Users/ny19205/OneDrive - University of Bristol/Documents - OneDrive/Mini-project4/01_Data/GWAS_tidy/nondense_area_unadj_GWAS_tidy_outcome.txt.gz")%>% select(SNP, beta_unadj = beta.outcome, pval_unadj = pval.outcome)

nda_mergedX<- full_join(nda_new_all, nda_unadj_all) %>% 
            filter(grepl("rs", SNP)) %>% 
            mutate(adj = ifelse(SNP %in% nda_new$SNP, 1, 0)) %>% 
            mutate(unadj = ifelse(SNP %in% nda_unadj$SNP, 1, 0)) %>% 
            filter(adj==1 | unadj==1)
write_tsv(nda_merged, paste0(results_path, current_trait_category, "/compare_NDA_adj_unadj.tsv"))
nda_merged<- read_tsv(paste0(results_path, current_trait_category, "/compare_NDA_adj_unadj.tsv"))


# pd
pd_new_all <- vroom("/Users/ny19205/OneDrive - University of Bristol/Documents - OneDrive/Mini-project4/01_Data/GWAS_tidy/percent_density_GWAS_tidy_outcome.txt.gz") %>% select(SNP, beta_adj = beta.outcome, pval_adj = pval.outcome)
pd_unadj_all <- vroom("/Users/ny19205/OneDrive - University of Bristol/Documents - OneDrive/Mini-project4/01_Data/GWAS_tidy/percent_density_unadj_GWAS_tidy_outcome.txt.gz")%>% select(SNP, beta_unadj = beta.outcome, pval_unadj = pval.outcome)

pd_merged<- full_join(pd_new_all, pd_unadj_all) %>% 
            filter(grepl("rs", SNP)) %>% 
            mutate(adj = ifelse(SNP %in% pd_new$SNP, 1, 0)) %>% 
            mutate(unadj = ifelse(SNP %in% pd_unadj$SNP, 1, 0)) %>% 
            filter(adj==1 | unadj==1)
write_tsv(pd_merged, paste0(results_path, current_trait_category, "/compare_PD_adj_unadj.tsv"))
pd_merged<- read_tsv(paste0(results_path, current_trait_category, "/compare_PD_adj_unadj.tsv"))
...
```{r}
### USINGTHIS ONE (see one down for using direct estimates)
...
dat_direct <- read_tsv(paste0(results_path, current_trait_category, "/mvmr/merged/merged_mvmr_earlyBMI-md_unadj-to-BCAC.tsv")) %>%  # made in 04
               ...
               mutate(outcome = factor(outcome, 
                                      levels = c("BCAC 2017","BCAC 2020",
                                                 ...
                                                 "Luminal A", "Luminal B1", "Luminal B2", 
                                                  "HER2-enriched", "TNBC" )),
                     outcome = factor(outcome, levels = rev(levels(outcome)))) %>% 
              mutate(effect = "Direct effect \n (accounted for childhood body size)")
  
dat_total <- dat_all2 %>% 
              mutate(effect = "Total effect") %>% distinct()
...
p<-ggplot(dat_merged,
  ...
  labs(color = "Outcome",y = "", x = "OR", shape="")+
  theme(legend.position = "right", legend.direction = "vertical", 
        plot.title.position = "plot")+
...
```{r}
# plot of CBMI direct effect on each BC type, accounted for MD phenos, VS total effect
