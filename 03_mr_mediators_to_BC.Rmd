...
full_join(
  da_old %>%  select(SNP, old_pval = pval.exposure),
  da_new %>%  select(SNP, new_pval = pval.exposure),
  by ='SNP') %>% View()
...
dat_all<- dat_all %>%
  ...
  mutate(outcome= case_when(outcome == 'Breast cancer' & id.outcome == 'ieu-a-1126' ~  'Breast cancer 2017',
                            ...
                            TRUE ~ outcome)) %>% 
  mutate(exposure = case_when(grepl("non-dense", exposure) ~ 'Non-dense area',
...
p<-ggplot(dat_all2,
           aes(y=exposure, x=or,  colour=outcome)) +
