...
dat <- read_tsv( paste0(results_path, current_trait_category, "/mvmr/merged/merged_mvmr_adultBMIearlyBMI-",current_trait_category,"-to-BCAC.tsv")) %>% 
  mutate(mediator = case_when(grepl("nondense_area", mediator) ~ "MD non-dense area + adult BMI unadj",
                              grepl("dense_area", mediator) ~ "MD dense area + adult BMI unadj",
                              grepl("percent_density", mediator) ~ "MD percent density + adult BMI unadj")) %>% 
