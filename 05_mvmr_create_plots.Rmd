...
all_data <- bind_rows(total_data, direct_data) %>% 
  ...
  mutate(exposure = factor(exposure, levels= rev(c(
    "Childhood body size",   "Childhood body size\n accounted for\n Dense area",   
...
#### DOES NOT WORK ATM - can figure out the stupif grid pacement -- see ggforest below! (tmp wokrs)

#qq<-ggplot(all_data, aes(y=rev(exposure), x=or, label=outcome, colour=outcome)) +
#  geom_errorbarh(aes(xmin=or_lci95, xmax=or_uci95), height=.3) +
#  geom_point(size=2)+
#  #geom_point(size=2, 
...
#  scale_y_discrete(position = "right")+
#  theme(strip.text = element_text(face = 'bold'))+
#  # version 1 facet_wrap(outcome ~ ., scale="free_y",  ncol = 1)+
#  # version 2 facet_wrap(exposure~ ., nrow=4)+
#  facet_grid(rows=vars(outcome), cols=vars(exposure), scales = "free") +
#  labs(color = "",y = "", x = "Odds ratio", subtitle="" )+
#  theme(legend.position = "none",
