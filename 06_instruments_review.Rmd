...
mutate(exposure_cat = case_when(
      ...
      grepl("breast", trait, ignore.case = T) ~ "Breast related",
...
snp_phewas_df3<- snp_phewas_df3 %>% mutate(label = case_when(exposure_cat == "Breast related" ~ "BC",
...
ggplot(data = snp_phewas_df3, mapping = aes(x = rsid, y = log10P_trunc, colour=cluster, shape= label, size=label)) + 
  ...
  theme_minimal_vgrid(9, rel_small = 1) +
  ...
  labs(title="",
       y = "-log10 p-value", x = "rsid / cluster", colour= "cluster")
