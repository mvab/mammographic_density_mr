...
cluster_df_genes<- cluster_df %>% dplyr::select(cluster=ID, geneFUMA) %>% distinct()
...
cluster_df_genes2<- cluster_df_tmp %>% 
                    dplyr::select(cluster=ID, geneFUMA, gprofiler_gene) %>% distinct()
```
...
cluster_df_genes3<- cluster_df_multi %>% 
  dplyr::select(cluster=ID, rsID, geneFUMA, gprofiler_gene, eQTL_gene, everything()) %>%
  distinct() 
