...
genestxt500kb <- read_tsv("../../genes/FUMA_job212193/genes.txt") ## test  -- this one finds OSR1
...
path_count2<- enriched_df_unw %>% select(Term, Genes) %>% distinct() %>% count(Term, sort=T) %>% rename(n_path = n)
...
```{r}
# continue playing with the output
enriched_df_unw <-  read_tsv("../../02_Results/tmp/DA_all_pathways_by_clusters.tsv")

enriched_df_unw %>% filter(grepl("epithel", Term)) %>% separate(Term, into=c("pathway", "rest"), sep = " \\(")  %>%  select(pathway, cluster) %>% distinct() %>% View()


enriched_df_unw %>% filter(clust_status == 'only in positive') %>% View()
