...
```{r}
pd_old <- read_tsv(paste0(data_path_tophits, "mammographic_density_PD_sieh_tophits.tsv")) 
nrow(pd_old) # 10

pd_new <- read_tsv(paste0(data_path_tophits, "percent_density_tophits.tsv")) 
nrow(pd_new) #16
...
out <- extract_outcome_data(snps = pd_new$SNP, outcome = "ieu-a-1126")
harmonised_new<- harmonise_data(exposure_dat = pd_new, outcome_dat = out)
...
out <- extract_outcome_data(snps = pd_old$SNP, outcome = "ieu-a-1126")
harmonised_old<- harmonise_data(exposure_dat = pd_old, outcome_dat = out)
...
#https://mrcieu.github.io/TwoSampleMR/articles/perform_mr.html#mr-steiger-directionality-test     ###### VIEW THIS
  ...
  bc_out <- extract_outcome_data(snps = pd_new$SNP, outcome = "ieu-a-1126")

  # harmonising
  exp_out <- harmonise_data(exposure_dat = pd_new, outcome_dat = out)
 ...
 #         r_xxo = 1, r_yyo = 1)
 #
  bc_out <- extract_outcome_data(snps = pd_old$SNP, outcome = "ieu-a-1126")

  # harmonising
  exp_out <- harmonise_data(exposure_dat = pd_old, outcome_dat = out)
  exp_out$samplesize.exposure <- 24192
  exp_out$samplesize.outcome <- 228951 
  exp_out$ncase.outcome = 122977
  exp_out$ncontrol.outcome = 105974
...
clust_plot_best_new = cluster_results_new$plots$two_stage +
  ggplot2::xlab("Genetic association with PD") +
...
save(snp_phewas, file="../../02_Results/cluster_phewas_PD.RData")
...
PD_snps_to_genes <- read_csv("../PD_snp_to_gene.csv")
cluster1_genes <- PD_snps_to_genes %>% filter(CLUSTER==1) %>% pull(final)
cluster2_genes <- PD_snps_to_genes %>% filter(CLUSTER==2) %>% pull(final)
cluster3_genes <- PD_snps_to_genes %>% filter(CLUSTER==3) %>% pull(final)
```
...
# GIANT
out <- extract_outcome_data(snps = pd_new$SNP, outcome = "ieu-a-974")


harmonised_bmi<- harmonise_data(exposure_dat = pd_new, outcome_dat = adult_bmi_ukb_gwas)
...
harmonised_fatmass<- harmonise_data(exposure_dat = pd_new, outcome_dat = fatmass_gwas)
...
harmonised_fatfreemass<- harmonise_data(exposure_dat = pd_new, outcome_dat = fatfreemass_gwas)
...
out <- extract_outcome_data(snps = pd_new$SNP, outcome = "ieu-a-1126") %>% filter(pval.outcome>5e-08) # remove SNPs that are associted with BC

pd_new_wo_bc <- pd_new %>% filter(SNP %in% out$SNP) # only keeping SNPs not associated with BC


out <- extract_outcome_data(snps = pd_new_wo_bc$SNP, outcome = "ieu-a-1126")
harmonised_new<- harmonise_data(exposure_dat = pd_new_wo_bc, outcome_dat = out)
harmonised_new %>% select(SNP, beta.exposure, beta.outcome, pval.exposure, pval.outcome) %>% View()
