...
```{r}
nda_old <- read_tsv(paste0(data_path_tophits, "mammographic_density_NDA_sieh_tophits.tsv")) 
nrow(nda_old) #

nda_new <- read_tsv(paste0(data_path_tophits, "nondense_area_tophits.tsv")) 
nrow(nda_new) #16

length(intersect(nda_old$SNP, nda_new$SNP)) # 11

nda_all <- read_tsv(paste0(data_path_tophits, "nondense_area_tophits_all.tsv")) #670
...
out <- extract_outcome_data(snps = nda_new$SNP, outcome = "ieu-a-1126")
harmonised_new<- harmonise_data(exposure_dat = nda_new, outcome_dat = out)
...
harmonised_new2 <- harmonised_new %>% filter(SNP != "rs7816345")
res_new2 <- mr(harmonised_new2) %>% 
  split_outcome() %>% 
  generate_odds_ratios()
...
out <- extract_outcome_data(snps = nda_old$SNP, outcome = "ieu-a-1126")
harmonised_old<- harmonise_data(exposure_dat = nda_old, outcome_dat = out)
...
out <- extract_outcome_data(snps = nda_all$SNP, outcome = "ieu-a-1126")
harmonised_all<- harmonise_data(exposure_dat = nda_all, outcome_dat = out)
...
x<-plot_grid(p1_new[[1]], p2_new[[1]], p3_new[[1]],
             ...
             labels = c('Extracted', '', "", "Table1"), nrow=2) 
...
#https://mrcieu.github.io/TwoSampleMR/articles/perform_mr.html#mr-steiger-directionality-test     ###### VIEW THIS
  ...
  bc_out <- extract_outcome_data(snps = nda_new$SNP, outcome = "ieu-a-1126")

  # harmonising
  exp_out <- harmonise_data(exposure_dat = nda_new, outcome_dat = out)
 ...
 #         r_xxo = 1, r_yyo = 1)
 #
  bc_out <- extract_outcome_data(snps = nda_old$SNP, outcome = "ieu-a-1126")

  # harmonising
  exp_out <- harmonise_data(exposure_dat = nda_old, outcome_dat = out)
  exp_out$samplesize.exposure <- 24192
  exp_out$samplesize.outcome <- 228951 
  exp_out$ncase.outcome = 122977
  exp_out$ncontrol.outcome = 105974
...
dim(ivwrad_new$outliers)[1]  #5 outliers at bonf 
...
harmonised_new_sub <- harmonised_new2 %>%  select("SNP", "exposure", "outcome",
                                       "beta.exposure", "se.exposure",
                                       "beta.outcome", "se.outcome")
cluster_data_new <- left_join(harmonised_new_sub, res_single_new) %>% drop_na()
...
clust_plot_best_new = cluster_results_new$plots$two_stage +
  ggplot2::xlab("Genetic association with NDA") +
...
# make membership matrix
cluster_df<-tibble()

for (cluster in names(cluster_results_new$cluster_membership)){
  
  cl_now <- unlist(cluster_results_new$cluster_membership[[cluster]]) %>% as.data.frame() %>% rownames_to_column() %>% rename(SNP = 2) %>% mutate(cluster_id = cluster)
  cluster_df <- bind_rows(cluster_df, cl_now)
}

cluster_df <- cluster_df %>% 
  pivot_wider(values_from = "rowname", names_from = "cluster_id", id_cols="SNP" )
...
harmonised_old_sub <- harmonised_old %>%  select("SNP", "exposure", "outcome",
                                       "beta.exposure", "se.exposure",
                                       "beta.outcome", "se.outcome")
cluster_data_old <- left_join(harmonised_old_sub, res_single_old) %>% drop_na()
...
write_tsv(cluster_df, "../../02_Results/tmp/NDA_all_snps_clusters.tsv")
...
#genestxt <- read_tsv("../../genes/FUMA_job213208/genes.txt")
snpstxt <- read_tsv("../../genes/FUMA_job213208/snps.txt")
#eqtltxt <- read_tsv("../../genes/FUMA_job213208/eqtl.txt")

cluster_df <- read_tsv( "../../02_Results/tmp/NDA_all_snps_clusters.tsv")


cluster_df <-cluster_df %>% left_join(snpstxt %>% dplyr::select(rsID, nearestGene), by = "rsID")

cluster_df_genes<- cluster_df %>% dplyr::select(cluster=ID, gene = nearestGene) %>% distinct()

cluster_df_genes %>% count(gene,sort=T)

### updading rsids that have been updated:
...
library(gprofiler2)
grof_res <- gsnpense(query =unique(cluster_df$rsID)) %>% dplyr::select(rsID=rs_id, gprofiler_gene = gene_names) %>% distinct()

cluster_df<- left_join(cluster_df,grof_res, by ="rsID")

cluster_df_genes2<- cluster_df %>% dplyr::select(cluster=ID, geneFUMA = nearestGene, gprofiler_gene) %>% distinct()

write_tsv(cluster_df_genes2, "../../02_Results/tmp/NDA_all_snps_clusters_gene_annotation.tsv")
...
length(unique(cluster_df$rsID)) #584
length(unique(eqtl1_sub$rsid)) # 390
...
eqtl1_sub_selected_tissues <- eqtl1 %>% filter(qtl_group %in% tissues_keep) %>% 
                                        dplyr::select(rsid, hgnc_symbol,qtl_group, median_tpm, pvalue, study_id) %>% distinct() %>% 
                                        filter(median_tpm > 0) 
...
cluster_df_genes3<- cluster_df %>% 
  dplyr::select(cluster=ID, rsID, geneFUMA = nearestGene, gprofiler_gene, eQTL_gene = hgnc_symbol, everything()) %>%
  distinct() %>% 
  mutate(eQTL_gene = ifelse(eQTL_gene=="", NA, eQTL_gene))

cluster_df_genes3_sub<- cluster_df_genes3 %>% dplyr::select("cluster" , "geneFUMA","gprofiler_gene" ,"eQTL_gene" ) %>% distinct()

write_tsv(cluster_df_genes3_sub, "../../02_Results/tmp/NDA_all_snps_clusters_gene_annotation_plus_eQTL.tsv")
...
xx <- get_pheno_assoc(snp="rs7816345")

# snps: 
snp_df<- unlist(clust_snp_list) %>% as_data_frame() ## OR
snp_df<- harmonised_new$SNP%>% as_data_frame()
...
save(snp_phewas, file="../../02_Results/cluster_phewas_NDA.RData")
...
genes_not_mapped
#[1] "RNA5SP56"      "AC012363.13"   "RP4-712E4.1"   "LINC01101"     "AC090453.1"   
# [6] "RP11-962G15.1" "RP11-419C23.1" "RP3-470L22.1"  "RP4-712E4.2"   "AC142293.3"   
#[11] "RP1-80N2.2"  
enriched_df_unw <-enriched_df_unw %>% left_join(cluster_df_genes2_sub, by  = c("Genes" = "gene"))

enriched_df_unw <-enriched_df_unw %>% 
                        mutate(clust_type = case_when(grepl("1|2|4|Null", cluster) ~ "negative",
                                                      grepl("3", cluster) ~ "positive")) 
...
write_tsv(enriched_df_unw, "../../02_Results/tmp/DA_all_pathways_by_clusters.tsv")
...
# GIANT
out <- extract_outcome_data(snps = nda_new$SNP, outcome = "ieu-a-974")


harmonised_bmi<- harmonise_data(exposure_dat = nda_new, outcome_dat = adult_bmi_ukb_gwas)
...
harmonised_fatmass<- harmonise_data(exposure_dat = nda_new, outcome_dat = fatmass_gwas)
...
harmonised_fatfreemass<- harmonise_data(exposure_dat = nda_new, outcome_dat = fatfreemass_gwas)
...
out <- extract_outcome_data(snps = nda_new$SNP, outcome = "ieu-a-1126") %>% filter(pval.outcome>5e-08) # remove SNPs that are associted with BC

nda_new_wo_bc <- nda_new %>% filter(SNP %in% out$SNP) # only keeping SNPs not associated with BC


out <- extract_outcome_data(snps = nda_new_wo_bc$SNP, outcome = "ieu-a-1126")
harmonised_new<- harmonise_data(exposure_dat = nda_new_wo_bc, outcome_dat = out)
harmonised_new %>% select(SNP, beta.exposure, beta.outcome, pval.exposure, pval.outcome) %>% View()
