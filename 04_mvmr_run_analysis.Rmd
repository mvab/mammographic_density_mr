...
adult_bmi_file <- data_lookup %>% filter(trait == "Adult BMI unadjust") %>% pull(tophits_data)### or this could be adult body size if used for MVMR on MD
...
adult_bmi_gwas_file <- data_lookup %>% filter(trait == "Adult BMI unadjust") %>% pull(full_data)
...
mvmr_types <- c( "mvmr_adultBMI-", "mvmr_adultBMIearlyBMI-")
